  ...
  return Object.assign(base, {
    layer: <E, R>(options: {
      ...
      base.layerWithCompiler({
        ...options,
        compiler: MssqlClient.makeCompiler(),
      }),
  });
};

export interface MsSqlDatabaseConstructor<DB, Self> extends Database.CoreDatabaseConstructor<
  DB,
  Self
> {
...
export const makeSqlClient = <DB>(options: {
  ...
  makeSqlClientBase({ ...options, compiler: MssqlClient.makeCompiler() });
